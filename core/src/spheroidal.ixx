...
// p^2 = -E_elec R^2/2 (E_elec electronic, no nuclear 1/R).
...
struct H2plusOrbital {
    ...
    std::vector<double> M;       // grid-normalized, sign(peak)>0
...
// Sturm count for symmetric tridiagonal (d=diag, b=off): eigenvalues strictly
// below e, via LDL^T pivot signs.
...
// k-th eigenvalue from the top (k=0 largest), by Sturm bisection.
inline double eig_from_top(const std::vector<double>& d,
    ...
    // k-th from top = (n-1-k)-th from bottom; want the largest e with
    // sturm_below <= n-1-k.
...
// Eigenvector for e by shifted inverse iteration (Thomas solves of the
// near-singular T - e).
...
// Angular operator (diag/off) for trial p^2; eigenvalue = A.
inline void angular_operator(double p2, int m, std::vector<double>& eta,
    ...
    for (int i = 0; i < kNeta; ++i) {
        const double etl = -1.0 + i * deta;
        const double eth = -1.0 + (i + 1) * deta;
        const double cl = (1.0 - etl * etl) * inv2;
        const double ch = (1.0 - eth * eth) * inv2;
...
// Radial GEP L Lambda = p^2 (xi^2) Lambda at fixed A, symmetrized by
// S=diag(xi); returns n_xi-th p^2 from the top, optional eigenvector Lambda.
inline double radial_p2(double A, int m, double R, int n_xi, double xi_max,
    ...
    std::vector<double> ld(kNxi), lb(kNxi - 1, 0.0);
    for (int i = 0; i < kNxi; ++i) {
        xi[i] = 1.0 + (i + 0.5) * dxi;
        s[i] = xi[i];  // sqrt(weight xi^2)
    }
    for (int i = 0; i < kNxi; ++i) {
        const double xl = 1.0 + i * dxi;
        const double xh = 1.0 + (i + 1) * dxi;
    ...
    // Symmetrize M = S^-1 L S^-1; eigenvalues = p^2.
...
// Solve one H2+ orbital at internuclear R.
inline H2plusOrbital h2plus_orbital(double R, int m, int n_eta, int n_xi) {
    using namespace spheroidal_detail;
    m = std::abs(m);
    // xi_max reaches kXiReach bohr: covers the display box so diffuse orbitals
    // aren't truncated.
    ...
    // Bracket: scan p^2 up for a sign change of g (g>0 at small p^2, g<0 once
    // p^2 overshoots).
    double lo = 1e-4;
    double glo = g(lo);
    double hi = lo;
    ...
    // Profiles at the converged (p^2, A).
    const double A = angular_A(p2, m, n_eta);
    ...
    // Sign: make the peak positive for reproducible display.
