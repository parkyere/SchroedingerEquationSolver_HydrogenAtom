...
//     d_ddot = -<r_ddot> = <grad V>,   (m = 1 a.u., electron charge -1)
// and the Larmor formula gives the instantaneous radiated power (atomic
// units, where 1/c^3 = alpha^3):
//     P = (2/3) alpha^3 |d_ddot|^2 = (2/3) alpha^3 |<grad V>|^2.
//
// This captures the COHERENT emission of a superposition; it is exactly 0 for
// a pure eigenstate (static density, zero mean force), whose spontaneous decay
// is a QED effect handled separately by the Einstein-A quantum jumps
// (core/decay.hpp). The two are complementary and consistent -- for a
// two-level superposition the power carries the SAME |<f|r|i>|^2 dipole matrix
// element that the Einstein A does.
...
// <grad V> = integral |psi|^2 grad V dr, with grad V by central differences on
// the periodic grid. By Ehrenfest this equals the dipole acceleration d_ddot
// of the electron cloud. (For the linear force of a harmonic well the central
// difference is exact, so <grad V> = w^2 <r> to machine precision.)
